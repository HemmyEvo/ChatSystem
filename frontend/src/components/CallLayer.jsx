...
const DrawingCanvas = ({ strokes, draftStroke, onPointerDown, onPointerMove, onPointerUp, remotePointer }) => {
  ...
  useEffect(() => {
    ...
    if (remotePointer?.x != null && remotePointer?.y != null) {
      const x = remotePointer.x * rect.width;
      const y = remotePointer.y * rect.height;
      ctx.fillStyle = '#f472b6';
      ctx.beginPath();
      ctx.arc(x, y, 6, 0, Math.PI * 2);
      ctx.fill();
    }
  }, [strokes, draftStroke, remotePointer]);
  ...
  return (
    <canvas
      ...
      onMouseDown={(event) => onPointerDown(normalizeEvent(event))}
      onMouseMove={(event) => onPointerMove(normalizeEvent(event))}
      onMouseUp={onPointerUp}
      onMouseLeave={onPointerUp}
      onTouchStart={(event) => onPointerDown(normalizeEvent(event))}
      onTouchMove={(event) => {
        event.preventDefault();
        onPointerMove(normalizeEvent(event));
      }}
...
function MediaTile({ stream, fallbackAvatar, title, subtitle, muted = false, mirrored = false, children }) {
  ...
  const hasVideoTrack = Boolean(stream?.getVideoTracks?.().some((track) => track.readyState === 'live' && track.enabled));

  return (
    <div className="relative overflow-hidden rounded-[28px] border border-white/10 bg-slate-950/70 shadow-2xl">
      {hasVideoTrack ? (
        <video ref={videoRef} autoPlay playsInline muted={muted} className={`h-full w-full object-cover ${mirrored ? 'scale-x-[-1]' : ''}`} />
      ) : (
        <div className="flex h-full min-h-[220px] w-full flex-col items-center justify-center bg-[radial-gradient(circle_at_top,#22d3ee22,transparent_50%),linear-gradient(135deg,#0f172a,#020617)] p-6 text-center">
        ...
        <div className="flex items-center justify-between gap-4">
          ...
          {remoteLocation && (
            <a
              ...
              className="hidden items-center gap-2 rounded-full border border-cyan-400/30 bg-cyan-400/10 px-4 py-2 text-sm text-cyan-100 transition hover:bg-cyan-400/20 md:inline-flex"
            >
              <MapPin size={16} />
              {remoteLocation.lat.toFixed(3)}, {remoteLocation.lng.toFixed(3)}
            </a>
          )}
        </div>
